...
class Hoosk_model extends CI_Model {
    ...
    function getPagesAll() {  
        ...
        // $this->db->join('hoosk_page_content', 'hoosk_page_content.pageID = hoosk_page_attributes.pageID');
        // $this->db->join('hoosk_page_meta', 'hoosk_page_meta.pageID = hoosk_page_attributes.pageID');
        $query = $this->db->get('arm_cms_page');
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
        return array();
    ...
    function getPage($id) {
        ...
        $this->db->select("*");
        $this->db->where("arm_cms_page.pageID", $id);
        // $this->db->join('hoosk_page_content', 'hoosk_page_content.pageID = hoosk_page_attributes.pageID');
        // $this->db->join('hoosk_page_meta', 'hoosk_page_meta.pageID = hoosk_page_attributes.pageID');
		$query = $this->db->get('arm_cms_page');
        if ($query->num_rows() > 0) {
    ...
    function removePage($id) {
        // Delete a page
        $this->db->delete('arm_cms_page', array('pageID' => $id));
    }
	...
	function countNavigation(){
        return $this->db->count_all('arm_navigation');
    }	
    ...
    function getAllNav($limit, $offset=0) {
        ...
        $this->db->select("*");
		$this->db->limit($limit, $offset);
        $query = $this->db->get('arm_navigation');
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
        return array();
    ...
    function getPageNav($url) {
        // Get the page details
        $this->db->select("*");
        $this->db->where("arm_cms_page.navTitle",$url);
	...
	function insertNav() {
        ...
        $delete_attr = '<a class="right" onclick="var li = this.parentNode; var ul = li.parentNode; ul.removeChild(li);"><i class="icon-remove"></i></a>';
        $search_attr = '<a class="right" xss=removed><i class="icon-remove"></i></a>';
		$navigationEdit = str_replace($search_attr, $delete_attr, $this->input->post('seriaNav'));
        ...
        $this->db->insert('arm_navigation', $data);
	}
   ...
   // exit;
		...
		$this->db->where("navSlug", $id);
        $this->db->update('arm_navigation', $data);
	}
	
	function removeNav($id) {
        // Delete a nav
        $this->db->delete('arm_navigation', array('navSlug' => $id));
    }
