                            ...
                            <div class="table-responsive export_content">
                                <table class="table table-striped table-hover" id="datatable2" cellspacing="0" width="100%">
                                    ...
                                    <tr>
                                        ...
                                        <?$paymem = $this->common_model->GetRow("MemberId='".$row->MemberId."'","arm_members"); ?>
                                        <td><?if($paymem){ if($paymem->UserName) { echo '<a href="'.base_url().'admin/customers/profile/'.$row->MemberId.'">'.$paymem->UserName.'</a>';} else{echo $paymem->FirstName;}} else{echo"--";}?></td>
                                        <?$payrec = $this->common_model->GetRow("MemberId='".$row->ReceiveBy."'","arm_members"); ?>
                                        <td><?if($payrec){echo $payrec->UserName;}else{echo"--";}?></td>
                                        <!-- <td><?php if($row->MPaymentAttachemt!='') {?>
                                            <a target="_blank" href="<?php echo base_url().$row->MPaymentAttachemt; ?>">
                                            <?if($row->MPaymentReference!=''){echo $row->MPaymentReference;}else{echo"--";} echo"</a>";}
                                            else{
                                        ...
                                        $package = $this->common_model->GetRow("PackageId='".$row->PackageId."'",$table); ?>
                                        <td><?if($package){echo $package->PackageName;} else{echo"---";}?></td>
                                        <td><?php echo  $CurrencySymbol." ".$row->PaymentAmount;?></td>
                                        <td><?php if($row->APaymentAttachemt!='') {?>
                                            <a target="_blank" href="<?php echo base_url().$row->APaymentAttachemt; ?>">
                                            <?if($row->APaymentReference!=''){echo $row->APaymentReference;}else{echo"--";} echo"</a>";}
                                            else{
                                            ...
                                            <div class="btn-group text-right">
                                                ...
                                                if($mlsetting->Id==9)
                                                {
                                                 ...
                                                 if($row->AdminStatus=='0'){?>
                                                    <li>
                                                        <a href="<?php echo base_url().'admin/mtapay/accept/'.$row->depositid;?>"><?php echo ucwords($this->lang->line('label_accept')); ?></a>
                                                    </li>
                                                    <li>
                                                        <a href="<?php echo base_url().'admin/mtapay/decline/'.$row->depositid;?>"><?php echo ucwords($this->lang->line('label_decline')); ?></a>
                                                    </li>
                                                    <?php } }else{
                                                    if($row->AdminStatus=='0'){?>
                                                    <li>
                                                        <a href="<?php echo base_url().'admin/mtapay/accept/'.$row->Payid;?>"><?php echo ucwords($this->lang->line('label_accept')); ?></a>
                                                    </li>
                                                    <li>
                                                        <a href="<?php echo base_url().'admin/mtapay/decline/'.$row->Payid;?>"><?php echo ucwords($this->lang->line('label_decline')); ?></a>
                                                    </li>


                                                    <?}}?>
                                                    <li>
                                                        <a href="<?php echo base_url().'admin/mtapay/delete/'.$row->Payid;?>"><?php echo ucwords($this->lang->line('label_delete')); ?></a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </td> 
