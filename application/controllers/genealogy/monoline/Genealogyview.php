...
if($this->session->MemberID!="")
 {
 	$userdet = $this->common_model->GetRow("MemberId='". $this->session->MemberID."' order by MonoLineId ASC Limit 0,1","arm_monolinematrix");

 	$userid =$userdet->MonoLineId;
 }
 else
 {
	$userid = '1';
...
public function user_ta($userid,$table)
{
	...
	<img src='.$this->rankimage($select_high->MemberId).' style=margin-bottom:-42px; />
      <h1>'.ucwords($this->getname($select_high->MemberId)).' ('.$select_high->MemberCount.') </h1></div>';
    	...
    	{ 
    		$gendata.='<ul class="collapse">';
    	    	...
    	    	for($i=0; $i<$mcount; $i++)
    	    	{
    	    		 	...
    	    		 	$incondition = "SpilloverId='".$select_member[$i]->MonoLineId."'";
    	    			...
    	    			if($inner_fata_count>0)
						{
						  ...
						  $gendata.= $this->call_from($select_member[$i]->MonoLineId,$table);
						}
						else
						{
			      ...
			      <h1>'.ucwords($this->getname($select_member[$i]->MemberId)).' ('.$select_member[$i]->MemberCount.')</h1>
			    </div></li>'; 
						}
...
public function call_from($userid,$table)
{
	...
	$gendatagof ="";
	 ...
	 $gendatagof.= '<li id="beer" class="expanded collapse" style="cursor: s-resize;"><div class="node"
	  data-toggle="tooltip" title=" UserName 		: '.ucwords($this->getname($select_high->MemberId)).' &#xa; Sponsor Name 	: '.ucwords($this->getspname($select_high->MemberId)).'&#xa; Member Since 	: '.$this->getdate($select_high->MemberId).'">
	  <img src='.$this->rankimage($select_high->MemberId).' style=margin-bottom:-42px; />
  				    <h1>'.ucwords($this->getname($select_high->MemberId)).'  ('.$select_high->MemberCount.')</h1>
   				 </div><ul>';
	 ...
	 for($i=0; $i<$run_count; $i++)
	 {
			...
			if($inner_fata_count >0)
			{
			 ...
			 <img src='.$this->rankimage($inner_fata->MemberId).' style=margin-bottom:-42px; />
			   ...
			   $gendatagof.= $this->user_ta1($inner_fata->MonoLineId,$table,$gendatagof);
			 $gendatagof.= '</li>';
			}
			else
			{
      ...
      <h1>'.ucwords($this->getname($inner_fata[$i]->MemberId)).' ('.$inner_fata[$i]->MemberCount.')</h1>
    </div></li>'; 
			}
	 }
