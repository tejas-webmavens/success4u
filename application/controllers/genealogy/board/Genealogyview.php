...
$this->data['val'] = $this->user_ta($userid,$table,$id);
//echo $this->data['val'];
...
public function user_ta($userid,$table,$bid)
{

	$gendata='';
	
	 $mcondition = "MemberId='".$userid."' AND BoardId='".$bid."' order by BoardMemberId asc limit 0,1";
	...
	<img src='.$this->rankimage($select_high->MemberId).' style=margin-bottom:-42px; />
      <h1>'.ucwords($this->getname($select_high->MemberId)).' ('.$this->count_downlines($select_high->BoardMemberId,$select_high->MemberCount,$table).') </h1></div>';

    $ncondition = "SpilloverId='".$select_high->BoardMemberId."'";
    	...
    	{ 
    		$gendata.='<ul class="collapse">';
    	    	...
    	    	for($i=0; $i<$mcount; $i++)
    	    	{
    	    		 	...
    	    		 	$incondition = "SpilloverId='".$select_member[$i]->BoardMemberId."'";
    	    			...
    	    			if($inner_fata_count>0)
						{
						  ...
						  $gendata.= $this->call_from($select_member[$i]->MemberId,$table,$bid);
						}
						else
						{
			      ...
			      <h1>'.ucwords($this->getname($select_member[$i]->MemberId)).' ('.$this->count_downlines($select_member[$i]->BoardMemberId,$select_member[$i]->MemberCount,$table).')</h1>
			    </div></li>'; 
						}
...
public function user_ta1($userid,$table,$bid)
{
...
$gendata="";
	...
	if($select_count>0)
	{
	  ...
	  {
			$uncondition = "SpilloverId= '".$select[$j]->BoardMemberId."'";
			...
			if($inner_fata_count>0)
			{
			  $gendata.= $this->call_from($select[$j]->MemberId,$table,$bid);
			}
			else
			{
				
			  $gendata.='<li id="beer" style="cursor: s-resize;"><img src='.$this->rankimage($inner_fata->MemberId).' style=margin-bottom:-42px; />
      <h1>'.ucwords($this->getname($inner_fata->MemberId)).' ('.$this->count_downlines($inner_fata->BoardMemberId,'',$table).') </h1>
    </li>'; 
			}
	  }
...
public function call_from($userid,$table,$bid)
{


	$gendatagof ="";
	 $cmcondition ="MemberId = '".$userid."' AND BoardId='".$bid."' order by BoardMemberId asc limit 0,1";
	 ...
	 data-toggle="tooltip" title=" UserName 		: '.ucwords($this->getname($select_high->MemberId)).' &#xa; Sponsor Name 	: '.ucwords($this->getspname($select_high->MemberId)).'&#xa; Member Since 	: '.$this->getdate($select_high->MemberId).'">
	  <img src='.$this->rankimage($select_high->MemberId).' style=margin-bottom:-42px; />
  				    <h1>'.ucwords($this->getname($select_high->MemberId)).'  ('.$this->count_downlines($select_high->BoardMemberId,$select_high->MemberCount,$table).')</h1>
   				 </div><ul>';
	 $cncondition = "SpilloverId = '".$select_high->BoardMemberId."'";
	 ...
	 {
	 		$cocondition ="BoardMemberId = '".$run[$i]->BoardMemberId."'";
			...
			if($inner_fata_count >0)
			{
			 ...
			 <img src='.$this->rankimage($inner_fata->MemberId).' style=margin-bottom:-42px; />
     					 <h1>'.ucwords($this->getname($inner_fata->MemberId)).' ('.$this->count_downlines($inner_fata->BoardMemberId,$inner_fata->MemberCount,$table).') </h1>
   					 </div>';
			   $gendatagof.= $this->user_ta1($inner_fata->BoardMemberId,$table,$bid);
			 $gendatagof.= '</li>';
			}
			else
			{
      ...
      <h1>'.ucwords($this->getname($inner_fata[$i]->MemberId)).' ('.$this->count_downlines($inner_fata[$i]->BoardMemberId,$inner_fata[$i]->MemberCount,$table).')</h1>
    </div></li>'; 
			}
	 }
