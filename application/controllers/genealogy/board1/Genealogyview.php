...
public function user_ta($userid,$table,$bid)
{
	...
	$gendata='';
   if($userid == '1' && $bid == '2'){  
	 $mcondition = "MemberId='".$userid."' AND BoardId='1' order by BoardMemberId asc limit 0,1";
	...
	<img src='.$this->rankimage($select_high->MemberId).' style=margin-bottom:-42px; />
      <h1>'.ucwords($this->getname($select_high->MemberId)).' ('.$this->count_downlines($select_high->BoardMemberId,0,$table).') </h1></div>';
    	...
    	{ 
    		$gendata.='<ul class="collapse">';
    	    	...
    	    	for($i=0; $i<$mcount; $i++)
    	    	{
			      ...
			      <h1>'.ucwords($this->getname($select_member[$i]->MemberId)).' ('.$this->count_downlines($select_member[$i]->BoardMemberId,'',$table).')</h1>
			    </div></li>'; 
						}
...
public function call_from($userid,$table,$bid)
{
	$gendatagof ="";
	 ...
	 data-toggle="tooltip" title=" UserName 		: '.ucwords($this->getname($select_high->MemberId)).' &#xa; Sponsor Name 	: '.ucwords($this->getspname($select_high->MemberId)).'&#xa; Member Since 	: '.$this->getdate($select_high->MemberId).'">
	  <img src='.$this->rankimage($select_high->MemberId).' style=margin-bottom:-42px; />
  				    <h1>'.ucwords($this->getname($select_high->MemberId)).'  ('.$this->count_downlines($select_high->BoardMemberId,'',$table).')</h1>
   				 </div><ul>';
	 ...
	 for($i=0; $i<$run_count; $i++)
	 {
      ...
      <h1>'.ucwords($this->getname($inner_fata[$i]->MemberId)).' ('.$this->count_downlines($inner_fata[$i]->BoardMemberId,'',$table).')</h1>
    </div></li>'; 
			}
	 }
