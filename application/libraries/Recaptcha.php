...
class Recaptcha
{
    ...
    public function __construct()
    {
        ...
        $CI =& get_instance();
        $CI->db->where('Page', 'reCaptcha');
        $CI->db->where('KeyValue', 'siteKey');
        $query=$CI->db->get('arm_setting');
        foreach($query->result_array() as $n):
            $this->_siteKey = $n['ContentValue'];
        endforeach;

        $CI =& get_instance();
        $CI->db->where('Page', 'reCaptcha');
        $CI->db->where('KeyValue', 'secretKey');
        $query=$CI->db->get('arm_setting');
        foreach($query->result_array() as $n):
            $this->_secretKey = $n['ContentValue'];
        endforeach;

        // $this->_siteKey = "6LdDVCATAAAAABB8GW1DtNzItSnhK3i7x_TUxoHn";
        // $this->_secretKey = "6LdDVCATAAAAAGdnE8Sj5mawbje3H9ttXaMR9YR1";
