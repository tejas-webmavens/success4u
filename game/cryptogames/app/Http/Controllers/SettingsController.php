...
class SettingsController extends Controller
{
    ...
    public function gamesSettings(Request $request, $id){


        $this->validate($request, [

            'ref_roll'=> 'required|numeric',
            'time_pay_roll' => 'required|numeric',
            'roll_level1' => 'required',
            'roll_level2' => 'required',
            'roll_level3' => 'required',
            'roll_level4' => 'required',
            'roll_level5' => 'required',
            'roll_level6' => 'required',
            'faucet_pay' => 'required',
            'faucet_time' => 'required|numeric',
            'ref_faucet' => 'required|numeric',
            'price_slot' => 'required',
        ...
        $settings->ref_roll = $request->ref_roll;
        $settings->time_pay_roll = $request->time_pay_roll;
        $settings->roll_level1 = $request->roll_level1;
        $settings->roll_level2 = $request->roll_level2;
        $settings->roll_level3 = $request->roll_level3;
        $settings->roll_level4 = $request->roll_level4;
        $settings->roll_level5 = $request->roll_level5;
        $settings->roll_level6 = $request->roll_level6;
        $settings->faucet_pay = $request->faucet_pay;
        $settings->ref_faucet = $request->ref_faucet;
        $settings->price_c_slot = $request->price_slot;

        $settings->save();


        session()->flash('message', 'Website Games Settings Has been Successfully Changes.');
        Session::flash('type', 'success');
        Session::flash('title', 'Update Successful');
